...
const ProductoController = {
	...
	totalItems: 0, // Total de productos encontrados
	totalPages: 0, // Total de páginas calculado
	...
	setupEvents() {
		...
		// Utilizar delegación de eventos para manejar clics en todo el documento
	...
	handleDocumentClick(event) {
		...
		if (target.classList.contains("page-link") || target.parentElement.classList.contains("page-link")) {
			...
			// Asegurarnos de capturar el elemento con data-page
	...
	async handleSearch(page = 1) {
		...
		const criteria = {
			...
			page: this.currentPage - 1, // Page empieza en 0 para el backend
	...
	goToPage(page) {
		...
		if (page >= 1 && page <= this.totalPages) {
			// Asegurar que la página se establece correctamente antes de buscar
			this.currentPage = page;
	...
	async handleUpdateProducto(event) {
		...
		try {
			...
			// Volver a la vista de búsqueda con resultados actualizados
			ProductoView.render("pro-inventario", "search"); // Renderizar formulario de búsqueda
			await this.handleSearch(); // Actualizar resultados
			this.setupEvents(); // Reconfigurar eventos
		} catch (error) {
	...
	async fetchProductoInfo(productId) {
		try {
			...
			if (updateForm && !updateForm.hasListener) {
	...
	async handleDeleteProduct(productId) {
		if (confirm("¿Estás seguro de que deseas eliminar este producto?")) {
			try {
				...
				await this.handleSearch(this.currentPage); // Refrescar después de eliminar
			} catch (error) {
